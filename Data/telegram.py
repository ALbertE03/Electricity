...
class ScraperT:
    def __init__(
        ...
        self.group_username: str = group_username or _get_env(
            "tg_group_username", "EmpresaElectricaDeLaHabana"
        )
    ...
    async def process_message_batch(
        ...
        entity,
        messages_batch,
        start_of_n_days_ago: int,
        end_of_today: int,
        extract_all: bool,
        extract_comments: bool = False,
    ) -> defaultdict[list]:
        """Procesa un lote de mensajes en paralelo y opcionalmente extrae comentarios"""
            ...
            for message in messages_batch:
                try:
                    ...
                    if hasattr(message, "reactions") and message.reactions:
                        if (
                            hasattr(message.reactions, "results")
                            and message.reactions.results
                        ):
                            for reaction in message.reactions.results:
                                if not hasattr(reaction, "reaction"):
                                    if hasattr(reaction.reaction, "emoticon"):
                                        ...
                                        reaction_type = (
                                            f"custom_{reaction.reaction.document_id}"
                                        )
                                    else:
                    ...
                    if (
                        ...
                        try:
                            ...
                            batch_data[year_month][-1]["comments"] = comments
                        except Exception as e:
                            print(
                                f"Error extrayendo comentarios del mensaje {message.id}: {e}"
                            )
    ...
    async def download_photo(self, message) -> str:
        ...
        try:
            ...
            photo_dir = os.path.join(
                "Data", "photos", self.group_username, year, month, day
            )
    ...
    async def extract_group_sms(
        ...
        async with TelegramClient("session_name", self.api_id, self.api_hash) as client:
            ...
            if not extract_all:
                ...
                start_of_n_days_ago = datetime.combine(
                    n_days_ago, datetime.min.time()
                ...
                print(
                    f"Extrayendo mensajes desde hace {n} días ({n_days_ago.strftime('%Y-%m-%d')}) hasta hoy ({today.strftime('%Y-%m-%d')})"
                )
            else:
            ...
            print(
                f"Procesando mensajes en lotes de {batch_size} con máximo {self.max_workers} hilos concurrentes..."
            )
                    ...
                    if len(messages_batch) >= batch_size:
                        ...
                        if len(processing_tasks) >= self.max_workers:
                            completed_batches = await asyncio.gather(
                                *processing_tasks[: self.max_workers]
                            )
            ...
            if filtered_count == 0 and not extract_all:
                ...
                print(
                    f"   Usa extract_all=True para extraer todos los mensajes disponibles."
                )
        ...
        def save_file(year_month, msgs):
            try:
                ...
                filename = os.path.join(
                    year_dir, f"mensajes_{self.group_username}_{year_month}.json"
                )
